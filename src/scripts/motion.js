/* ==========================================================================
   ...
   ========================================================================== */
const hosts = document.querySelectorAll(".page-hero, .section--black");
...
if (hosts.length && "IntersectionObserver" in window) {
  ...
  // --fx-p runs from -1 to 1 across a black section, and from 0 to 1 as the hero leaves the top.
  const move = () => {
    frame = 0;
    const h = window.innerHeight;
    const boxes = [...onScreen].map((el) => [el, el.getBoundingClientRect()]); // read all, then write
    boxes.forEach(([el, r]) => {
      const p = el.classList.contains("page-hero")
